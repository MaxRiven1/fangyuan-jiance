...
(async()=>{
  ...
  const CLICK=`(()=>{const a=Array.from(document.querySelectorAll('a,span,div')).find(e=>(e.textContent||'').trim()==='总价');if(!a)return 'NO_TOTAL_BTN';a.click();return 'CLICKED';})()`;
  ...
  await sleep(500);
  console.log('default first3=',JSON.stringify(await evaluate(EXTRACT)));
  await evaluate(CLICK); await sleep(3000);
  console.log('after click1  =',JSON.stringify(await evaluate(EXTRACT)));
  await evaluate(CLICK); await sleep(3000);
  console.log('after click2  =',JSON.stringify(await evaluate(EXTRACT)));
