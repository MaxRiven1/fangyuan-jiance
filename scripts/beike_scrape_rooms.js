...
// 仅抓取"新候选小区"（已在池的9个保留今日巡检数据不重抓）
// 直接用 co21l3rs / co21l2rs 价格升序+户型过滤URL（已验证可排除车位、纯住宅）
const COMMUNITIES = [
  ...
  { id: 'JN-SY-0', name: '后子门', search: '后子门', school: '实验小学本部', district: '青羊区', area: '后子门(老破小代表)' },
  { id: 'JN-PT-0', name: '支矶石街', search: '支矶石街', school: '泡桐树小学本部', district: '青羊区', area: '泡桐树街(老破小代表)' },
  { id: 'JN-SX-0', name: '包家巷', search: '包家巷', school: '胜西小学', district: '青羊区', area: '包家巷(老破小代表)' },
  { id: 'JN-RB-0', name: '白马寺街', search: '白马寺街', school: '人民北路小学', district: '金牛区', area: '白马寺(老破小代表)' }
];
...
(async () => {
  ...
  const EXTRACT = `(() => {
    ...
    for (const li of items) {
      ...
      out.push({ title: titleEl.textContent.trim(), href: titleEl.href, house: houseEl.textContent.trim().replace(/\\s+/g, ' '), position: posEl ? posEl.textContent.trim().replace(/\\s+/g, ' ') : '', totalPrice: tpEl.textContent.trim(), unitPrice: upEl ? upEl.textContent.trim() : '' });
    }
    return out;
  })()`;
  ...
  for (const c of COMMUNITIES) {
    ...
    for (const room of ['l3', 'l2']) {
      ...
      process.stderr.write('>> [' + c.school + '] ' + c.name + ' co21' + room + '\n');
      await send('Page.navigate', { url });
      await sleep(2500);
      const ok = await waitFor(`!!document.querySelector('.sellListContent > li') || /没有找到|暂无房源/.test(document.body.innerText)`, 18000);
      if (!ok) { continue; }
      let data;
      try { data = await evaluate(EXTRACT); } catch (e) { continue; }
      const kw = c.search;
      const filtered = data.filter(it => (it.position + ' ' + it.house + ' ' + it.title).includes(kw));
      rec[room] = filtered;
      await sleep(2000 + Math.floor(Math.random() * 2000));
    }
    results[c.id] = rec;
  }
  fs.writeFileSync(process.argv[2] || 'beike_rooms_result.json', JSON.stringify(results, null, 2), 'utf8');
  console.log('DONE, saved to ' + (process.argv[2] || 'beike_rooms_result.json'));
